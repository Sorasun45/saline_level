...
function FluidMeter() {
  ...
  var bubblesLayer = {
    ...
    reset: function (bubble) {
      ...
      var fluidAmount = currentFillPercentage * (getMeterRadius() - options.borderWidth * 2) / options.maxValue;
  ...
  function drawFluidLayer(layer, dt) {
    ...
    var meterBottom = (options.size - (options.size - getMeterRadius()) / 2) - options.borderWidth;
    //! max value change at : currentFillPercentage * (getMeterRadius() - options.borderWidth * 2) / maxValue;
    var fluidAmount = currentFillPercentage * (getMeterRadius() - options.borderWidth * 2) / options.maxValue;

    //! currentFillPercentage ความเร็วในการเพิ่มของน้ำ
    // ความไวของตัวเลข/ระดับน้ำ
    const fillSpeed = 15;
    if (currentFillPercentage < fillPercentage) {
      currentFillPercentage += fillSpeed * dt;
  ...
  function drawText() {
    ...
    // แสดงผลเป็นตัวเลขจำนวนเต็ม ไม่มีทศนิยม
    // ✅ แสดงค่าแบบเต็มตาม maxValue เช่น 500/1000
    var text = Math.round(currentFillPercentage) + " ml. ";
  ...
  return {
    init: function (env) {
      ...
      //! clamp(number, 0, maxValue)
      options.maxValue = env.options.maxValue || options.maxValue;
      fillPercentage = clamp(env.fillPercentage, 0, options.maxValue);
      if (env.options) {
        options.maxValue = env.options.maxValue;
    ...
    setPercentage(percentage) {
      //! clamp(number, 0, maxValue)
      fillPercentage = clamp(percentage, 0, options.maxValue);
    }
  }
};
